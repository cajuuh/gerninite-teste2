...
export class CanvasChartComponent implements OnInit {
  ...
  drawSpline(ctx, pts, t, closed, showDetails, width, height) {
    if (closed) {
      this.drawSplineClosed(ctx, pts, t, width, height);
    } else this.drawSplineOpen(ctx, pts, t, width, height);
  ...
  drawSplineClosed(ctx, pts, t, width?, height?) {
    ctx.lineWidth = 4;
    ctx.save();
    var cp = [];
    var n = pts.length;
  ...
  drawSplineOpen(ctx, pts, t, width?, height?) {
    ctx.lineWidth = 4;
    ctx.save();
    var cp = [];
    var n = pts.length;
  ...
  multiAnimate() {
    this.animate(0.5);
    //    animate(1,() =>{animate(0,() =>{animate(0.33333)})});
  }
  ...
  getPoints(width, height, percCarregamento, pontoSelecionado) {
    ...
    if (percCarregamento >= 0)
      ptsClone.forEach((value, index) => {
        value.y =
          this.pts[index].y * percCarregamento +
          (1.0 - percCarregamento) * height;
        value.isSelecionado = pontoSelecionado
          ? pontoSelecionado.indexSelecionado == index - 2
          : false;
        value.diametroSelecionado = pontoSelecionado
          ? pontoSelecionado.diametroSelecionado
          : 0;
      });
    return ptsClone;
  }
  ...
  main(t, percCarregamento, pontoSelecionado?) {
    ...
    this.drawSpline(
      ctx,
      this.getPoints(e.width, e.height, percCarregamento, pontoSelecionado),
      t,
      true,
      percCarregamento == 1,
      e.width,
    ...
    var row = <HTMLElement>document.querySelector("row");
    if (row) {
      row[0].style.background = "url(" + e.toDataURL() + ")";
      row[1].style.background = "url(" + e.toDataURL() + ")";
      row[2].style.background = "url(" + e.toDataURL() + ")";
    }
  }
    ...
    for (var i = 0; i <= qtdFrames; i += 1) {
      (() => {
        ...
        setTimeout(() => {
          ...
          this.main(t2, 1, {
            indexSelecionado,
            diametroSelecionado: diametroProporcional
          });
        }, tempoDeEspera);
      })();
    }
